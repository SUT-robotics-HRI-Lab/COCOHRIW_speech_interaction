...
class DialogManagerNode(Node):
    def __init__(self):
        ...
        self.banter_sub = self.create_subscription(String, '/transcription_classifier/banter', self.banter_callback, 10)
    ...
    def clarification_callback(self, msg: String):
        ...
        self.llm_interface.request_task_field_extraction(user_input, missing_fields, on_extracted_fields)

    def process_task(self, task: TaskInformation, task_index: int):

        if task.task_type == "Unknown":
            self.get_logger().warn("🚫 Task type is UNKNOWN. Skipping this task.")
            req = Speak.Request()
        ...
        state_msg.previous_state = self.current_dialog_state
        if is_valid:
            if self.current_chunk_tasks:
                state_msg.current_state = "AWAITING_CONFIRMATION"
            else:
                state_msg.current_state = "CONFIRMING_TASK"
        else:
            state_msg.current_state = "AWAITING_CLARIFICATION"
        self.dialog_state_pub.publish(state_msg)
        ...
        if is_valid:
            ...
            self.expecting_clarification = False
            self.process_next_task_if_any()
            if self.current_chunk_tasks and len(self.completed_chunk_tasks) == len(self.current_chunk_tasks):
                self.get_logger().info("📦 All tasks from the current array completed. Publishing TaskArray.")
                task_array_msg = TaskArray()
                task_array_msg.tasks = self.completed_chunk_tasks
                self.task_array_complete_pub.publish(task_array_msg)
                self.current_chunk_tasks = []
                self.completed_chunk_tasks = []
        else:
    ...
    def task_array_callback(self, msg: TaskArray):
        if self.processing_task or not self.task_queue.empty():
            ...
            return
        self.current_chunk_tasks = msg.tasks
        self.completed_chunk_tasks = []
    ...
    def banter_callback(self, msg: String):
        if self.current_dialog_state == "AWAITING_CLARIFICATION":
            self.get_logger().info("💬 Banter received during clarification. Reminding user about needed info.")
            # Re-ask the clarification question
            if self.last_incomplete_index is not None:
                _, clarification_sentence, _ = self.task_handler.register_task(
                    self.task_handler.get_task(self.last_incomplete_index),
                    self.last_incomplete_index
                )
        ...
        elif self.current_dialog_state == "AWAITING_TASK":
            self.get_logger().info("💬 Banter received while idle. Generating a friendly idle response.")
            banter_input = msg.data.strip()
            prompt_text = self.llm_interface.build_banter_while_idle_prompt(banter_input)

            def on_banter_response(response_text, error):
                if error:
                    self.get_logger().error(f"LLM Banter Error: {error}")
                    return
                self.get_logger().info(f"🗨️ LLM Response to banter: {response_text}")
                req = Speak.Request()
                req.text = response_text
                future = self.tts_client.call_async(req)
                future.add_done_callback(lambda f: self.get_logger().info("✅ TTS idle banter response sent."))

            self.llm_interface.request_clarification_text(prompt_text, on_banter_response)
        else:
            self.get_logger().info("💬 Banter received but ignored due to dialog state: " + self.current_dialog_state)
